    ...
    struct ScannerContainerView: View {
        ...
        private var qrView: some View {
            ZStack {
                ...
                VStack {
                    Spacer()
                    animatedClueLabels(count: qrClueURLs.count, lit: discoveredClues)
                }
            }
        }
        ...
        private func animatedClueLabels(count: Int, lit: Set<Int>) -> some View {
            HStack(spacing: 16) {
                ForEach(1...count, id: \.self) { idx in
                    VStack(spacing: 4) {
                        // Clue number with enhanced styling
                        Text("\(idx)")
                    ...
                    .onAppear {
                        ...
                        withAnimation(.easeOut(duration: 0.5).delay(Double(idx) * 0.1)) {
                            // Animation handled by the main animation above
                        }
                    }
                }
            }
